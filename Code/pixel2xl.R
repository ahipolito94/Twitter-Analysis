...
# step 1: import data
pixel_df <- read.csv("/Users/ahipolito94/Capstone_2/Data/pixel2xl.csv", header = TRUE, encoding = "UCS-2LE")

# step 2: clean tweets/pre-processing
pixel_df$clean_tweets <- cleanTweets(pixel_df)            
new_df <- pixel_df[!duplicated(pixel_df$clean_tweets), ]  # removes duplicates
dim(pixel_df)
dim(new_df)
pixel_num <- nrow(new_df)

# step 3: sentiment analysis
new_df$sentiment <- get_sentiment(new_df$clean_tweets)

pixel_sent_min <- min(new_df$sentiment)
pixel_sent_max <- max(new_df$sentiment)
pixel_sent_mean <- mean(new_df$sentiment)
pixel_sent_median <- median(new_df$sentiment)

# step 4: tokenize words on space and output as a list
doc.list <- strsplit(pixel_df$clean_tweets, "[[:space:]]+")
...
stop_words <- c(stopwords("english"), "twitter", "href", "via", "amp", "rt", "retweet", "twitter", "tweet", 
                "pixelxl", "pixel", "xl", "false", "b", "iphonex", "phone", "smartphones", "google",
                "bd")
