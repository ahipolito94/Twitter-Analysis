...
oneplus_df$clean_tweets <- cleanTweets(oneplus_df)            
new_df <- oneplus_df[!duplicated(oneplus_df$clean_tweets), ]  # removes duplicates
dim(oneplus_df)
dim(new_df)
oneplus_num <- nrow(new_df)

# step 3: sentiment analysis
new_df$sentiment <- get_sentiment(new_df$clean_tweets)

oneplus_sent_min <- min(new_df$sentiment)
oneplus_sent_max <- max(new_df$sentiment)
oneplus_sent_mean <- mean(new_df$sentiment)
oneplus_sent_median <- median(new_df$sentiment)

# step 4: tokenize words on space and output as a list
doc.list <- strsplit(new_df$clean_tweets, "[[:space:]]+")
...
stop_words <- c(stopwords("english"), "twitter", "href", "via", "amp", "rt", "retweet", "tweet", 
                "oneplus", "one", "plus", "false", "b", "giveaway", "star", "wars", "bd", 
                "oneplust", "android", "qualif", "phone")
del <- names(term.table) %in% stop_words | term.table < 5
term.table <- term.table[!del]

# step 7: frequency plot
